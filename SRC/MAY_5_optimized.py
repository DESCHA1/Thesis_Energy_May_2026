...
for horizon in horizons:
    ...
    for fold, (train_idx, val_idx) in enumerate(tscv.split(df_trainval)):
        ...
        for run in range(CONFIG["robustness_runs"]):
            ...
            # ------------------------------------------------------------------
            # FEATURE IMPORTANCE  ← FIXED: save plot + CSV for every run 0
            # ------------------------------------------------------------------
            if run == 0:
                ...
                # --- save plots for every importance type ---
                figs = tft.plot_interpretation(interpretation)
                for fig_name, fig in figs.items():
                    plot_path = (
                        f"DATA/optimized_importance_{horizon}h"
                        f"_fold{fold+1}_{fig_name}.png"
                    )
                    try:
                        # plot_interpretation() can return either a Figure or
                        # an Axes object depending on the pytorch-forecasting
                        # version — handle both gracefully
                        figure = fig if hasattr(fig, "savefig") else fig.get_figure()
