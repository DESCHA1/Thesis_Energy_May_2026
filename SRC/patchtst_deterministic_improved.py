...
class ImprovedPatchTST(nn.Module):
    ...
    fused and decoded by an MLP point-forecast head.
    """
    def __init__(self, n_past_ch: int, n_known_ch: int,
        ...
        self.head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(d_model, horizon),   # point forecast: H values, not H*Q
        )
...
def evaluate(model, loader):
    ...
    for x_past, x_fut, yb in loader:
        x_past, x_fut, yb = (x_past.to(DEVICE), x_fut.to(DEVICE),
                              yb.to(DEVICE))
        pred        = model(x_past, x_fut)
        total_loss += criterion(pred, yb).item() * len(x_past)
...
def train_model(model, tr_loader, va_loader,
    ...
    for epoch in range(max_epochs):
        ...
        print(f"  Epoch {epoch+1:02d} | train_mse={tr_loss:.4f} | "
              f"val_mse={va_loss:.4f}", flush=True)
...
# ==============================================================================
# 8. DATA PREP  (identical 90/10 split to TFT and QR-PatchTST)
# ==============================================================================
...
@torch.no_grad()
def extract_attention_importance(model: ImprovedPatchTST,
                                 val_df: pd.DataFrame,
                                 encoder_len: int, horizon: int,
                                 label: str, fold: int):
    attn_maps = []
